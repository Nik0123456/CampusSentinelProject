$(function() {
  ...
  function updateAnalyzer(data) {
    ...
    m_process.gauge('update', {value: 100 * (data.cpuLoadProcess || 0)});
    m_system.gauge('update', {value: 100 * (data.cpuLoadSystem || 0)});
  ...
  function updateApps(appInfo) {
    ...
    $.get('../scripts/json', function(scriptInfo) {
      ...
      var diff = apps.length - $('#app-list').children().length;
      if(diff > 0) {
        for(let i = 0; i < diff; i++) {
          $('#app-list').append('<a href="#" class="btn m-1 text-truncate disabled"></a>');
        }
      } else if (diff < 0) {
        for(let i = 0; i > diff; i--) {
          $('#app-list').find('a:first').remove();
        }
      }

      $('#app-list a').each(function(i) {
        var button = $(this);
        let rec = appInfo[apps[i]];
        let href = rec.entry ? '../app/'+apps[i]+rec.entry  : '#';
        if(button.attr('href') !== href) {
          button.attr('href', href);
        }
        if(failedApps[apps[i]]) {
