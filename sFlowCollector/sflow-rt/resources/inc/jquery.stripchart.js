// Copyright (c) 2014-2025 InMon Corp. ALL RIGHTS RESERVED
...
(function ($) {
    ...
    function maxValue(values, sumFlag, mirrorFlag) {
        ...
        for (i = 0; i < values[0].length; i++) {
            ...
            if (mirrorFlag) {
                if (maxdown > maxDown)
                    maxDown = maxdown;
                if (sumdown > maxSumDown)
                    maxSumDown = sumdown;
            }
        }
        maxUp = sumFlag ? maxSumUp : maxUp;
        maxDown = sumFlag ? maxSumDown : maxDown;
    ...
    function drawTimeAxis(ctx, h, w, maxVal, option, times, step, selectedIdx) {
        var stepsizes = [5000, 10000, 15000, 60000, 120000, 300000, 600000, 900000, 1200000, 1800000, 3600000, 7200000, 10800000, 14400000, 21600000, 43200000, 86400000];
        var yBase = h - option.bottomInset;
        var tmin = times[0];
        var tmax = times[times.length - 1];
        if (step)
            tmax += (tmax - tmin) / Math.max(times.length - 1, 1);
        ...
        while (tick < tmax) {
            if (tick >= tmin) {
                ...
                if(tick > tmin) drawLine(ctx, xPos, yBase, xPos, option.topInset, "#ccc");
                ctx.fillText(timeStr(tick, stepsize < 60000), xPos, tMiddle);
            }
            tick += stepsize;
        }
        ...
        if (times && times[selectedIdx]) {
            includeSeconds = (tdelta / Math.max(times.length,1)) < 30000;
    ...
    function drawStackedSeries(ctx, h, w, maxVal, yZero, vscale, tmin, tmax, tscale, option, values, times, step, mirror, colors) {
        ...
        if (mirror && maxVal.down)
            drawLine(ctx, option.leftInset, yZero, w - option.rightInset, yZero, "#000");
    }
    ...
    function drawSeries(ctx, h, w, maxVal, yZero, vscale, tmin, tmax, tscale, option, values, times, step, mirror, colors) {
        ...
        for (s = values.length; --s >= 0; ) {
            ...
            if (mirror) {
                col = colors[Math.floor(s / 2) % colors.length];
                if (s % 2)
                    ctx.strokeStyle = darkenColor(col);
                else
                    ctx.strokeStyle = col;
            } else {
    ...
    function addLegend(chart, h, w, option, legendInfo, values, mirror, colors) {
        ...
        if (legendInfo.headings) {
            ...
            for (i = 0; i < legendInfo.headings.length; i++)
                legend += '<th>' + legendInfo.headings[i] + '</th>';
            legend += '</tr></thead>';
        }
        function createLink(idx, val) {
            ...
            if (!link)
                return val;
            link = link.replace(/\{(.*?)\}/g, function (match, tok) {
                return val;
            });
            ...
            for (r = 0; r < legendInfo.labels.length; r++) {
                ...
                if (legendInfo.labels[r] instanceof Array) {
                    ...
                    for (c = 0; c < row.length; c++)
                        legend += '<td>' + createLink(c, row[c]) + '</td>';
                } else
                    legend += '<td>' + createLink(r, legendInfo.labels[r]) + '</td>';
                legend += '</tr>';
            }
            legend += '</tbody>';
        }
    ...
    $.widget('inmon.stripchart', {
        ...
        draw: function (cdata) {
            ...
            if (cdata.hrule) {
                for (i = 0; i < cdata.hrule.length; i++) {
                    if (cdata.hrule[i].scale) {
                        ...
                        if (maxVal.up < hval)
                            maxVal.up = hval;
                        if (mirror && maxVal.down < hval)
                            maxVal.down = hval;
                    }
                }
            }
            if (maxVal.up === 0 && maxVal.down === 0)
                maxVal.up = 1;
            ...
            ctx.clip();
            if (stack)
                drawStackedSeries(ctx, h, w, maxVal, vMetrics.yZero, vMetrics.vScale, tMetrics.tMin, tMetrics.tMax, tMetrics.tScale, this.options, cdata.values, cdata.times, step, mirror, colors);
            else
                drawSeries(ctx, h, w, maxVal, vMetrics.yZero, vMetrics.vScale, tMetrics.tMin, tMetrics.tMax, tMetrics.tScale, this.options, cdata.values, cdata.times, step, mirror, colors);
            if (cdata.hrule)
                drawHrule(ctx, h, w, maxVal, vMetrics.yZero, vMetrics.vScale, this.options, cdata.hrule);
            ctx.restore();
            if (cdata.legend)
                addLegend(this, h, w, this.options, cdata.legend, cdata.values, mirror, colors);
        }
    });
})(jQuery);
