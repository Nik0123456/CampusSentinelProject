...
# -*- coding: utf-8 -*-
"""
sflow_test_monitor_v3.py (CORREGIDO)
Monitor DDoS usando flows cargados vía API REST
"""
...
def print_header():
    """Banner del script"""
    print("="*80)
    print("  CAMPUS SENTINEL - sFlow DDoS Monitor v3")
    print("="*80)
...
def check_flows():
    ...
    CORRECCIÓN: Usar /flow/json (sin 's') para obtener definiciones
    """
    try:
        r = requests.get(f"{SFLOWRT_URL}/flow/json", timeout=3)  # ← CORREGIDO
        if r.status_code == 200:
            flows = r.json()
            return flows
        return {}
...
def get_active_flows(agent, flow_name, n=10):
    ...
    Endpoint: /activeflows/{agent}/{flow_name}/json
    """
    try:
        r = requests.get(
...
def display_flows_table(flows, title, value_type="bytes"):
    """Muestra tabla formateada de flows"""
    print(f"\n{'─'*80}")
    print(f"  {title}")
    print(f"{'─'*80}")
    ...
    for flow in flows[:10]:
        try:
            ...
            print(f"  Error procesando flow: {e}")
    
    print()

def analyze_anomalies(agent):
    """Analiza anomalías en el tráfico"""
    anomalies = []
            ...
            if anomalies:
                for anomaly in anomalies:
                    ...
                    if anom_type == 'BANDWIDTH_HIGH':
                        print(f"🚨 BANDWIDTH ALTO: {flow} → {value:. 2f} {unit}")
                    elif anom_type == 'PPS_HIGH':
                        print(f"🚨 PPS ALTO: {flow} → {value:,.0f} {unit}")
                    elif anom_type == 'TALKER_HIGH':
                        print(f"⚠️  TALKER SOSPECHOSO: {flow} → {value:.2f} {unit}")
            else:
